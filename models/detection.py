...
from NFPA.nfpa import NFPA
from NFPA import nfpa
import os
import math
...
w, h, c = 224, 224, 3
batch_size = nfpa.batch_size
total_class = nfpa.total_class
...
def convert_rev(size, box):
    ...
    top =   (cy - ch / 2.) * h;
    bot =   (cy + ch / 2.) * h;
    box = map(int,[label,left,top,right,bot])
    return list(box)
...
def cust_loss(y_pred, y_true)
    loss = tf.square( y_pred[1] - y_true[1] )+ tf.square( y_pred[2] - y_true[2] ) + \
           tf.square( y_pred[3] - y_true[3]) + tf.square(y_pred[4] - y_true[4])
    loss = max(x, 0) - x * z + log(1 + exp(-abs(x)))
    return loss
cross_entropy = tf.nn.sigmoid_cross_entropy_with_logits(logits=Ylogits, labels=Y_)
cross_entropy = tf.reduce_mean(cross_entropy) * 100

cross_entropy = cust_loss(logits=Ylogits, labels=Y_)
cross_entropy = tf.reduce_mean(cross_entropy) * 100
...
if train:
    ...
    for i in range(10000):
        ...
        if i % 10 == 0:
            ...
            print(str(i) + ": ********* epoch " + str(i * 100 // len(data.labels)) + " ********* test accuracy:" + str(
                a) + " test loss: " + str(c))

        if i % 100 == 0:
            saver.save(sess, './detection')
    ...
    img = cv2.imread(r"NFPA/images/pos-1.jpg")
    ...
    # generate csv file
    testImgPath = r"/home/shahrukh/sk/dnn/tf/muti_label/ML_learn/models/NFPA/images/"
    ...
    for i, imgName in enumerate(images):
        ...
        rect = convert_rev((w,h),Y_out[0][0][:])
        print (rect)
